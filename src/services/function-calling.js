...
class FunctionCallingSystem {
  ...
  // =====================================================
  // CONTACT LINKING (LEAD CAPTURE)
  // =====================================================

  async linkContactIdentity({ identityType, identityValue, platformType, platformUserId }) {
    try {
      if (!identityType || !identityValue || !platformType || !platformUserId) {
        throw new Error('Missing required parameters');
      }

      // Ensure there is a conversation and get its id
      const conv = await this.dbService.getOrCreateConversation(platformUserId, platformType);
      const contact = await this.dbService.upsertContactWithIdentity(identityType, String(identityValue), null, 'ai_tool');
      await this.dbService.linkConversationToContact(conv.conversation_id, contact.contact_id);

      return {
        function: 'linkContactIdentity',
        status: 'success',
        data: { conversation_id: conv.conversation_id, contact_id: contact.contact_id }
      };
    } catch (error) {
      return { function: 'linkContactIdentity', status: 'error', error: error.message };
    }
  }
  ...
  getAvailableFunctions() {
    return [
      {
        name: "linkContactIdentity",
        description: "Link a contact identity (email/phone/platform ID) to the active conversation for dedup",
        parameters: {
          type: "object",
          properties: {
            identityType: { type: "string", enum: ["email", "phone", "instagram", "facebook", "webchat_session"], description: "Type of identity" },
            identityValue: { type: "string", description: "Value of the identity" },
            platformType: { type: "string", enum: ["instagram", "facebook", "facebook_messenger", "webchat"], description: "Platform for the conversation" },
            platformUserId: { type: "string", description: "Platform user identifier (IGSID, PSID, session id)" }
          },
          required: ["identityType", "identityValue", "platformType", "platformUserId"]
        },
        category: "contacts"
      },
      // Core API Functions
  ...
  async executeFunction(functionName, parameters) {
    try {
      ...
      switch (functionName) {
        case 'linkContactIdentity':
          return await this.linkContactIdentity(parameters);
